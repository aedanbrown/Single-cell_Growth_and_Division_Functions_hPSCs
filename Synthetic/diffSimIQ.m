...
tauV_ = 1:1:200;
n_dist_ = 50000;
pd0_ = makedist('Normal','mu',50,'sigma',0.5);
pd0_ = truncate(pd0_,0,100);

%PSF functions
Gamma_ = @(x) (x./100).^5;
R_ = @(x) (50 + x.^2)./(100 + x.^2) - 5e-3.*x;
D_ = @(x) 0.5./(1 + 5e-3.*exp(0.5.^2.*x));


[X_i,C_p_t,C_d_t,n_d,n_nb,n_diff,tauV] = IQ_diff(N_p_,tauV_,pd0_,Gamma_,R_,D_,n_dist_);


writematrix(X_i,sprintf('Xi_%s.csv',fName));
writematrix(C_p_t,sprintf('Cpt_%s.csv',fName));
writematrix(C_d_t,sprintf('Cdt_%s.csv',fName));
writematrix(n_d,sprintf('nd_%s.csv',fName));
writematrix(n_nb,sprintf('nnb_%s.csv',fName));
writematrix(n_diff,sprintf('ndiff_%s.csv',fName));
writematrix(tauV',sprintf('tauV_%s.csv',fName));


function [X_i,C_p_t,C_d_t,n_d,n_nb,n_diff,tauV] = IQ_diff(N_p,tauV,pd0,Gamma,R,D,n_dist)

    C_p = N_p; %Initial pluripotent cell concentration
    C_d = 0; %Initial differentiated cell concentration
    T_d = 0; %Initial differentiated cell count
...
%     %Generate initial cell count distribution
    ...
    %Initial IQ
    [T_0_cdf,x_i_T,~] = calculate_T_diff(tau_0,x_i_0,R,Gamma,D);
    ...
    action = divOrdiff(x_i_tau,Gamma,D);

    if isequal(action, 'div')

        mother = cdf_sample_disc(@(j) mother_cdf_div(j,x_i_tau,Gamma),[1 N_p]);
        %Select mother cell for divion
    ...
    elseif isequal(action, 'diff')

        mother = cdf_sample_disc(@(j) mother_cdf_diff(j,x_i_tau,D),[1 N_p]);
        %Select mother cell for divion

        x_mother = x_i_tau(mother); %Find mother cell content

        if n_diffi <= n_dist
            n_diff(n_diffi) = x_mother;
            n_diffi = n_diffi + 1;
        else
            n_diff(randi(n_dist)) = x_mother;
        end
        ...
        x_i_tau(mother) = randsample(x_i_tau(1:N_p ~= mother),1);
            %Replace the differentiated cell with another random cell from
    ...
    while tau < tauMax
        ...
        [T,x_i_T,~] = calculate_T_diff(tau,x_i_tau,R,Gamma,D); %Calculate IQ length
        ...
        elseif isequal(action,'diff')
            ...
            mother = cdf_sample_disc(@(j) mother_cdf_diff(j,x_i_tau,D),[1 N_p]);
            %Select mother cell for differentiation
...
function [M] = mother_cdf_div(j,x_i,Gamma)
...
function [M] = mother_cdf_diff(j,x_i,D)
    
    M = zeros(length(j),1);
    for i = 1:length(j)
        M(i) = sum(D(x_i(1:j(i))))./sum(D(x_i));
    end

end


function [action] = divOrdiff(x,Gamma,D)

    p = sum(Gamma(x))./(sum(Gamma(x)) + sum(D(x))); %Probability of division
...
function [T_,x_i_T_,u] = calculate_T_diff(tau,x_i_tau,R,Gamma,D)

    u = rand(1,1);
    Ti = 0;
    [Qi,dQdTi,x_i_T_] = Q_dQdT(Ti,u,tau,x_i_tau,R,Gamma,D);
    ...
    while abs(Qi) > 1e-6
        ...
        if dQdTi ~= 0 %Confirm derivatie is nonzero
            ...
            [Qip1,dQdTip1,x_i_T_] = Q_dQdT(Tip1,u,tau,x_i_tau,R,Gamma,D);
...
function [q,dqdt,x_i_T] = Q_dQdT(T,u,tau,x_i_0,R,Gamma,D)
    ...
    elseif T < 0
        ...
        dqdt = alpha(x_i_0',Gamma,D)./log(1-u);
...
function [a] = alpha(x_i,Gamma,D)
    
    a = sum(Gamma(x_i),2) + sum(D(x_i),2);
