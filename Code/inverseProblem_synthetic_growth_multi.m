function [] = inverseProblem_synthetic_growth_multi(fName,nFiles,Gamma,R)
    ...
    if any(logical(tauV - mean(tauV,3)))
        ...
        exportgraphics(gcf,sprintf("g_%s_%i_muFit_multi.png",fName,tMin))
        savefig(sprintf("f_%s_%i_muFit_multi.fig",fName,tMin))
        ...
        fprintf("Number of total cells for data %i: %.1f\n",nFiles,length(nT_temp))
